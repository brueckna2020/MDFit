...
def cross_validation(model:object, X:ndarray, y:ndarray, search_params:dict, 
    ...
    **kwargs : dict
    ...
    for train_index, test_index in tqdm(LeaveOneOut().split(X), desc=desc):
        # Preprocess
        if scale:
...
def get_subset(array, matches):
    return np.argwhere([v in matches for v in array]).flatten()

def molecule_cross_validation(model, X, y, search_params, ids=None, importance=None, 
                     features=None, cv=-1, n_jobs=1, scale=True, desc='Molecule CV', 
                     **kwargs):
    results = {'pred':[], 'obs':[], 'ids':[]}
    if importance is not None and features is not None:
        results['imp'] = {name:[] for name in features}
    unique_ids = np.array(list(set(ids)))
    for train_mol_index, test_mol_index in tqdm(LeaveOneOut().split(unique_ids), desc=desc):
        train_index = get_subset(ids, unique_ids[train_mol_index])
        test_index = get_subset(ids, unique_ids[test_mol_index])
        # Preprocess
        if scale:
            target_scaler = StandardScaler()
            y_train = y[train_index].reshape(len(train_index), 1)
...
def leave_one_feature_out_importance(workflow, cv_method=cross_validation, 
                                     nested=False, cv=-1, n_jobs=1, **kwargs):
...
class BaseWorkflow(base.Savable, metaclass=ABCMeta):
    def __init__(self, base_model=None, data=None, target_col=None, id_col='ID',
                 search_params=None, scale=True, **kwargs):
        # Loading
    ...
    def _get_data(self, unstandardized=False):
        X = np.array(self.X)
        y = np.array(self.y)
    ...
    def fit(self, n_jobs=1, cv=-1, **kwargs):
    ...
    def cross_validate(self, cv_method=molecule_cross_validation, nested=False, cv=-1, 
                       n_jobs=1, **kwargs):
...
class LinearRegression(BaseWorkflow):
    ...
    def __init__(self, data, target_col, id_col='ID', **kwargs):
        parameters = None
    ...
    @staticmethod
    def _feature_importance(model, features):
        return pd.Series(
